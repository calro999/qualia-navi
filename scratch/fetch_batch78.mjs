...
// 検索ボリューム特大・年中需要・切実ロングテール
const groups = [
  { 
    group: 1, 
    keywords: ["ピュアビタミンC 美容液", "ビタミンC25 美容液", "グルタチオン 美容液 白玉", "高濃度ビタミンC 毛穴"] 
  },
  { 
    group: 2, 
    keywords: ["ウォーターピーリング 美顔器", "超音波ピーリング 毛穴", "ウォーターピーリング 防水", "超音波 美顔器 毛穴洗浄"] 
  },
  { 
    group: 3, 
    keywords: ["ヘマチン ヘアマスク", "エルカラクトン トリートメント", "髪質改善 ヘアマスク うねり", "ヘマチン トリートメント"] 
  },
  { 
    group: 4, 
    keywords: ["バーガンディ リップ ブルベ", "チャコール アイシャドウ", "ブルベ冬 リップ ダーク", "バーガンディ 口紅"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 78 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 78 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch78_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 78!");
}
