...
async function main() {
  const keyword = "タカミスキンピール 角質 美容液";
  console.log(`Fetching items for: ${keyword}...`);
  const encoded = encodeURIComponent("角質美容液 スキンピール");
  ...
  fs.writeFileSync("scratch/rakuten_batch115_g4_verified.json", JSON.stringify(items, null, 2));
  console.log(`Saved ${items.length} items to scratch/rakuten_batch115_g4_verified.json`);
}
