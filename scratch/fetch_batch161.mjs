...
const queries = {
  t1: '着圧ソックス むくみ おやすみ用',
  t2: '着圧レギンス 美脚 脚痩せ 補正',
  t3: 'スカルプシャンプー 女性 育毛 ハリコシ',
  t4: '足指 セパレーター 外反母趾 シリコン'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch161_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 161 products saved successfully to scratch/batch161_fetched.json!');
}
