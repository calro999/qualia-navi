...
const queries = {
  t1: '舌スクレーパー 銅 ステンレス 舌磨き 舌苔',
  t2: 'ニキビパッチ ハイドロコロイド 薄型 目立たない',
  t3: 'レチノール ボディミルク ボディローション ボディクリーム',
  t4: '二の腕 ブツブツ 石鹸 尿素 サリチル酸 毛孔性苔癬'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch146_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 146 products saved successfully to scratch/batch146_fetched.json!');
}
