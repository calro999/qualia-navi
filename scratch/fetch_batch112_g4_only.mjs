...
async function main() {
  console.log("Fetching items for g4: モーブ アイシャドウ パレット...");
  const encoded = encodeURIComponent("モーブ アイシャドウ パレット");
  ...
  fs.writeFileSync("scratch/rakuten_batch112_g4_verified.json", JSON.stringify(items, null, 2));
  console.log(`Saved ${items.length} items to scratch/rakuten_batch112_g4_verified.json`);
}
