...
const queries = [
  {
    group: "g1",
    keyword: "アゼライン酸 美容液 毛穴 赤み",
    minPrice: 1500,
  },
  {
    group: "g2",
    keyword: "ウォーターピーリング 美顔器 超音波 毛穴",
    minPrice: 2500,
  },
  {
    group: "g3",
    keyword: "ヘマチン スカルプ 頭皮 美容液 育毛",
    minPrice: 2000,
  },
  {
    group: "g4",
    keyword: "アイシャドウ コーラル アプリコット イエベ春",
    minPrice: 1000,
  }
];
...
async function main() {
  for (const q of queries) {
    ...
    fs.writeFileSync(`scratch/rakuten_batch92_${q.group}_verified.json`, JSON.stringify(items, null, 2), "utf8");
    console.log(`Saved ${items.length} items for ${q.group}`);
    await sleep(1500);
  }
  console.log("All queries fetched for Batch 92!");
}
