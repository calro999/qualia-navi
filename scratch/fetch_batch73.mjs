...
// 検索ボリューム極大・年中需要・切実ロングテール
const groups = [
  { 
    group: 1, 
    keywords: ["トラネキサム酸 美容液", "美白美容液 シミ 肝斑", "トラネキサム酸 原液", "アルブチン 美容液"] 
  },
  { 
    group: 2, 
    keywords: ["クレンジングバーム 毛穴", "酵素 クレンジングバーム", "クレンジング 毛穴 角栓", "クレンジングバーム 温感"] 
  },
  { 
    group: 3, 
    keywords: ["女性用 育毛剤 薬用", "育毛剤 女性 薄毛 抜け毛", "スカルプエッセンス 女性用 育毛", "育毛ローション 女性"] 
  },
  { 
    group: 4, 
    keywords: ["イエベ秋 リップ シナモン", "テラコッタ リップ", "イエベ秋 チーク ベージュ", "メープル リップ イエベ"] 
  }
];
...
async function searchRakuten(keyword) {
  ...
  url.searchParams.set("genreId", "100938"); // コスメ・美容
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 73 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 73 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch73_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 73!");
}
