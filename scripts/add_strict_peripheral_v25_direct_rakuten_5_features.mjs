...
// ① クエリ：角質削り かかと やすり ガラス フットファイル, ドクターショール かかと 角質（ストッキングが引っかかる頑固なガサガサかかと・ひび割れを特殊ナノガラスや両面粗微細ブレードで削り落とし、赤ちゃんのようなツルスベ素足に戻すフットケアやすり）
// ② クエリ：頭皮 ブラシ シャンプーブラシ シリコン スカルプ, ウカ スカルプブラシ ケンザン（シャンプー時に指先では落とせない毛穴の皮脂汚れを適度な硬さのシリコン突起で揉み出し、心地よい刺激で頭皮のコリをほぐすスカルプブラシ）
const NEW_PERIPHERAL_V25_FEATURES = [
  {
    featureId: 'feature-5-glass-foot-file-heel-care',
    title: '【2026年最新・楽天最安値】【かかと角質削り＆ナノガラスフットファイル】ストッキングの引っかかりゼロ！頑固なガサガサかかとを一掃する「神角質やすり」5選',
    category: 'bodycare',
    categoryLabel: '🦶 【神かかと角質削り5選】ナノガラス＆ツルスベ素足特集',
    introText: '「かかとが白くひび割れて痛い・靴下に引っかかる」「削りすぎて皮膚を傷つけたくない」…特殊ナノレーザーエッチングガラス、両面粗微細ステンレス、人間工学グリップ採用で力を入れずに撫でるだけで古い角質だけを粉状に削り落とす楽天売れ筋フットファイル5選を徹底検証！',
    items: [
      { id: 'art-footfile-dr-scholl-velvet-smooth', keyword: 'ドクターショール かかと 角質', label: 'かかとケアの世界的定番！頑固な硬化角質をなめらかに整える安心のブランド' },
      { id: 'art-footfile-nano-glass-heel-eraser', keyword: 'かかと やすり ガラス ナノ', label: '撫でるだけで角質が粉雪のように削れる！水洗い丸洗いで半永久的に使えるナノガラス' },
      { id: 'art-footfile-double-sided-stainless-file', keyword: 'かかと 角質取り 両面 やすり', label: '粗面と細面の2WAY！頑固なタコから仕上げのツルスベ磨きまで1本で完結' },
      { id: 'art-footfile-five-skin-glass-foot-polisher', keyword: 'かかと 削り ガラス', label: '肌を傷つけず角質だけをターゲット！お風呂でも乾いた足でも使える万能設計' },
      { id: 'art-footfile-foot-care-heel-pedicure-kit', keyword: 'フットケア 角質 かかと削り', label: '人間工学に基づいたカーブ形状で力を入れずにかかと全体を均一にケア' }
    ]
  },
  {
    featureId: 'feature-5-silicone-scalp-shampoo-brush',
    title: '【2026年最新・楽天最安値】【シリコン頭皮マッサージブラシ＆シャンプー用ケンザン】毛穴の奥の皮脂をかき出し頭皮スッキリ「神スカルプブラシ」5選',
    category: 'haircare',
    categoryLabel: '💆‍♂️ 【神シリコン頭皮ブラシ5選】シャンプー毛穴洗浄＆コリほぐし特集',
    introText: '「爪を立てずに頭皮の毛穴の奥までしっかり洗いたい」「シャンプーしながら美容室のヘッドスパのような爽快感を味わいたい」…絶妙な硬さの高品質シリコン突起、手にフィットする蝶々形状・水抜け穴設計で頭皮のコリをほぐしながらニオイとベタつきを一掃する楽天大人気スカルプブラシ5選を徹底解説！',
    items: [
      { id: 'art-shampbrush-uka-scalp-brush-kenzan', keyword: 'uka スカルプブラシ ケンザン', label: '頭皮ブラシの絶対王者！絶妙な硬さのシリコン突起で美容賢者がこぞって愛用する名品' },
      { id: 'art-shampbrush-etvos-relaxing-massage-brush', keyword: 'エトヴォス リラクシングマッサージブラシ', label: '皮脂詰まりをオフしながらツボを刺激！握りやすく女性の手にフィットする大人気ブラシ' },
      { id: 'art-shampbrush-silicone-head-spa-cleaner', keyword: '頭皮ブラシ シャンプー シリコン', label: '弾力のあるシリコン突起が毛穴の汚れをスッキリかき出す！お風呂上がりの頭皮マッサージにも' },
      { id: 'art-shampbrush-hard-kenzan-scalp-deep-press', keyword: 'スカルプブラシ 硬め', label: 'しっかりした刺激が欲しい方に！頭皮のコリを力強くほぐすハードタイプ' },
      { id: 'art-shampbrush-soft-sensitive-scalp-massage', keyword: '頭皮 マッサージ ブラシ やわらかめ', label: '敏感肌・細い髪でも安心！地肌を優しく包み込んで血行を促すソフトシリコン' }
    ]
  }
];

async function main() {
  console.log('🚀 楽天公式OpenAPI直接通信（フォールバック一切禁止・周辺クエリ第25弾）を開始します...');

  const articlesJsonPath = path.resolve(process.cwd(), 'src/data/articles.json');
  let articles = JSON.parse(fs.readFileSync(articlesJsonPath, 'utf-8'));

  for (const feat of NEW_PERIPHERAL_V25_FEATURES) {
    console.log(`\n==================================================`);
    ...
    for (let i = 0; i < feat.items.length; i++) {
      ...
      const singleArticle = {
        ...
        pros: [
          `使用後すぐにかかとの頑固なガサガサ解消・ツルスベ化や頭皮の毛穴の皮脂オフ・爽快ヘッドスパ感を実感できる高機能設計`,
          `毎日のバスタイムやフットケア習慣に無理なく組み込める快適な使用感・高い満足度`,
...
2. **肌や頭皮への優しさと持続性**: デリケートな部位でも毎日心地よく使える安心の品質管理。
...
- **安心の正規品保証**: 信頼できる認証ショップからの購入で偽物リスクを完全回避。`,
        ...
        reviewerRole: 'フットファイル＆スカルプブラシケアアナリスト',
        summaryKeyPoints: [
    ...
    const featureArticle = {
      ...
      reviewerRole: 'シニアフットケア＆スカルプブラシケアアナリスト',
      summaryKeyPoints: [
  ...
  console.log(`\n🎉 楽天API直接取得（フォールバック一切禁止）による周辺クエリ第25弾特集記事および個別商品記事の完全追加が完了しました！`);
}
