...
async function fetchMore(targetFile, keywords, minPrice = 800) {
  let existing = JSON.parse(fs.readFileSync(targetFile, "utf8"));
  ...
  console.log(`Now ${targetFile} has ${existing.length} items`);
}

async function main() {
  // G1: PHA ピーリング 美容液 角質ケア
  await fetchMore("scratch/rakuten_batch97_g1_verified.json", [
    "ピーリング 美容液 角質ケア マイルド",
    "水光 ピーリング 美容液 韓国",
    "角質 導入 美容液 敏感肌"
  ], 1000);

  // G4: アイシャドウ パープル モーブ ブルベ
  await fetchMore("scratch/rakuten_batch97_g4_verified.json", [
    "アイシャドウ パープル パレット",
    "アイシャドウ モーブ ブルベ冬",
    "アイシャドウ パレット ワイン"
  ], 700);

  console.log("Done supplementing Batch 97!");
}
