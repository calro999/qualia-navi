...
const groups = [
  { 
    group: 1, 
    keywords: ["酵素洗顔 パウダー 毛穴", "酵素洗顔パウダー", "クレイ 洗顔パウダー", "酵素洗顔 個包装"] 
  },
  { 
    group: 2, 
    keywords: ["プラズマ 美顔器", "プラズマシャワー 美顔器", "プラズマ クル 美顔器", "美顔器 プラズマ"] 
  },
  { 
    group: 3, 
    keywords: ["アルガンオイル ヘアオイル", "マルラオイル ヘア", "ヘアオイル パサつき 補修", "アルガンオイル トリートメント"] 
  },
  { 
    group: 4, 
    keywords: ["オレンジ リップ イエベ春", "アプリコット リップ", "オレンジ 口紅", "コーラルオレンジ リップ"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 75 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 75 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch75_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 75!");
}
