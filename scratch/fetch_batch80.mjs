...
async function run() {
  // G1: セラミド クリーム 保湿 バリア
  const g1 = await fetchRakutenItems('セラミド クリーム 保湿 バリア', 10);
  await sleep(1500);

  // G2: 電気ブラシ EMS 美顔器 頭皮
  const g2 = await fetchRakutenItems('電気ブラシ EMS 美顔器 頭皮', 10);
  await sleep(1500);

  // G3: 炭酸シャンプー ヘッドスパ スカルプ
  const g3 = await fetchRakutenItems('炭酸シャンプー ヘッドスパ スカルプ', 10);
  await sleep(1500);

  // G4: アイシャドウ ラベンダー ブルベ夏
  const g4 = await fetchRakutenItems('アイシャドウ ラベンダー ブルベ夏', 10);

  fs.writeFileSync('scratch/rakuten_batch80_g1_verified.json', JSON.stringify(g1, null, 2));
  fs.writeFileSync('scratch/rakuten_batch80_g2_verified.json', JSON.stringify(g2, null, 2));
  fs.writeFileSync('scratch/rakuten_batch80_g3_verified.json', JSON.stringify(g3, null, 2));
  fs.writeFileSync('scratch/rakuten_batch80_g4_verified.json', JSON.stringify(g4, null, 2));
