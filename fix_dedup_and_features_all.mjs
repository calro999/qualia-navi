...
console.log('🚀 [All 10-Article Deduplicator & Feature Enhancer] 重複排除・真の特徴＆キャッチコピー生成開始...');
...
// 楽天APIから直接商品を取得し、厳格に重複を排除して10商品を揃える
async function fetchRakutenDistinctItems(keywords, minHits = 10) {
  const distinct = [];
  ...
  for (const kw of keywords) {
    ...
    try {
      const url = `https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260401?applicationId=${APP_ID}&accessKey=${ACCESS_KEY}&affiliateId=${AFFILIATE_ID}&keyword=${encodeURIComponent(kw)}&format=json&hits=30&sort=-reviewCount&imageFlag=1`;
      ...
      for (const e of data.Items) {
        ...
        // 同一判定チェック (商品名のコア、画像ファイル名、アフィリエイト先URL)
        ...
        for (const seen of seenCores) {
          if (seen.length >= 6 && coreKey.length >= 6 && (seen.includes(coreKey) || coreKey.includes(seen))) {
            isDuplicate = true;
            break;
          }
        }
...
async function main() {
  ...
  for (const art of content10s) {
    ...
    console.log(`\n----------------------------------------`);
    console.log(`🔨 [${processed + 1}/${content10s.length}] 重複排除＆真の特徴更新: [${art.id}] ${art.title.slice(0, 35)}...`);

    // 検索用キーワードリストの作成
    const cleanTitle = art.title.replace(/【.*?】/g, '').replace(/[！!？?・&％%]/g, ' ').replace(/\d+選.*/, '').trim();
    ...
    const items = await fetchRakutenDistinctItems(keywords, 10);
    if (items.length < 10) {
    ...
    console.log(`✅ 重複完全ゼロ・魅力的な特徴付きで10商品更新完了！ (1位: ${items[0].itemName.slice(0, 25)}...)`);
    processed++;
  ...
  console.log(`\n🎉 全${processed}件の10選記事において重複の完全排除＆本物の特徴・キャッチコピー反映が完了しました！`);
}
