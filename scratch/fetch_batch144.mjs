...
const queries = {
  t1: '歯ブラシ 除菌 ケース UV 携帯 充電式',
  t2: '足つぼマット 足裏 マッサージ 足つぼ マット',
  t3: 'シリコン 洗顔ブラシ 指 小鼻 毛穴 ブラシ',
  t4: 'ドライヤー フード キャップ ボンネット 速乾'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch144_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 144 products saved successfully to scratch/batch144_fetched.json!');
}
