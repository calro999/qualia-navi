...
async function fetchTo10(targetFile, keywords, minPrice = 1000) {
  let existing = [];
...
async function main() {
  // G1: アイクリーム レチノール
  await fetchTo10("scratch/rakuten_batch93_g1_verified.json", [
    "レチノール アイクリーム 目元 シワ",
    "エクソソーム アイクリーム 目元 たるみ",
    "アイクリーム 目元 レチノール"
  ], 1500);

  // G3: ヘアミスト アイロン ヒートプロテクト
  await fetchTo10("scratch/rakuten_batch93_g3_verified.json", [
    "ヘアミスト アイロン ヒートプロテクト",
    "エルカラクトン ヘアミスト",
    "ヘアウォーター ストレート アイロン"
  ], 1000);

  // G4: アイシャドウ ボルドー バーガンディ
  await fetchTo10("scratch/rakuten_batch93_g4_verified.json", [
    "アイシャドウ ボルドー",
    "アイシャドウ バーガンディ",
    "アイシャドウ パレット ワインレッド ブルベ"
  ], 800);

  console.log("All supplemented successfully!");
}
