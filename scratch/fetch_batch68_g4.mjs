...
const keywords = ["コーラル リップ", "コーラルピンク チーク", "コーラル リップティント", "イエベ春 リップ"];
...
async function run() {
  ...
  for (const kw of keywords) {
    ...
    await sleep(1500); // 1.5s delay to avoid 429
  ...
  console.log(`>>> Group 4 total verified: ${valid.length}`);
  fs.writeFileSync(`scratch/rakuten_batch68_g4_verified.json`, JSON.stringify(valid, null, 2));
}
