...
const queries = {
  t1: 'メガネ くもり止め クロス 拭くだけ マスク',
  t2: 'シャンプーバー 固形シャンプー エシカル 石鹸',
  t3: 'セラミック プッシャー 甘皮 オイルイン ペン',
  t4: 'リップ スリーピングマスク 夜用 リップマスク 唇'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch147_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 147 products saved successfully to scratch/batch147_fetched.json!');
}
