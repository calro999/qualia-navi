...
async function run() {
  // G1: アゼライン酸 美容液 クリーム
  const g1 = await fetchRakutenItems('アゼライン酸 美容液 クリーム', 10);
  await sleep(1500);

  // G2: キャビテーション 美顔器 痩身 EMS
  const g2 = await fetchRakutenItems('キャビテーション 美顔器 痩身 EMS', 10);
  await sleep(1500);

  // G3: まつ毛美容液 まつ毛ケア
  const g3 = await fetchRakutenItems('まつ毛美容液 まつ毛ケア', 10);
  await sleep(1500);

  // G4: アイシャドウ コーラル イエベ春
  const g4 = await fetchRakutenItems('アイシャドウ コーラル イエベ春', 10);

  fs.writeFileSync('scratch/rakuten_batch81_g1_verified.json', JSON.stringify(g1, null, 2));
  fs.writeFileSync('scratch/rakuten_batch81_g2_verified.json', JSON.stringify(g2, null, 2));
  fs.writeFileSync('scratch/rakuten_batch81_g3_verified.json', JSON.stringify(g3, null, 2));
  fs.writeFileSync('scratch/rakuten_batch81_g4_verified.json', JSON.stringify(g4, null, 2));
