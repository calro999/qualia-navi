...
console.log('🚀 [楽天API 直接取得 Batch6] 超高需要ロングテール5テーマの商品データを取得開始...');
...
const themes = [
  {
    key: 'bangs_glue',
    searchKeywords: ['前髪 グルー マトメイク', '前髪 固定 のり', 'マトメイク 前髪グルー ウテナ', '前髪キープ グルー']
  },
  {
    key: 'green_concealer',
    searchKeywords: ['グリーン コンシーラー 赤み', '緑 コントロールカラー ニキビ跡', 'ザセム グリーン コンシーラー', '小鼻 赤み消し コンシーラー']
  },
  {
    key: 'eyebrow_stamp',
    searchKeywords: ['アイブロウ スタンプ 眉', 'ニューアイブロウスタンプ キス', '眉毛 スタンプ 時短', '眉スタンプ テンプレート']
  },
  {
    key: 'fork_eyebrow_pen',
    searchKeywords: ['4股 アイブロウ ティント', 'フォーク型 アイブロウ 眉毛', '4本ライン リキッド 眉', '毛並み アイブロウ ペン']
  },
  {
    key: 'cool_contour_greige',
    searchKeywords: ['シェーディング ブルベ グレージュ', 'ペリペラ シェーディング グレージュ', 'アートクラスバイロダン モダン', '影色 シェーディング ノーズシャドウ']
  }
];
...
async function main() {
  ...
  fs.writeFileSync('scratch_longtail_batch6_rakuten_items.json', JSON.stringify(results, null, 2), 'utf-8');
  console.log('\n🎉 全商品の取得が完了し、scratch_longtail_batch6_rakuten_items.json に保存しました！');
}
