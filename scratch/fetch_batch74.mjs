...
// 検索ボリューム特大・高単価・切実ロングテール
const groups = [
  { 
    group: 1, 
    keywords: ["レチノール アイクリーム", "シワ改善 アイクリーム", "レチノール クリーム 目元", "ナイアシンアミド リンクルクリーム"] 
  },
  { 
    group: 2, 
    keywords: ["脱毛器 冷却", "IPL 脱毛器 VIO", "光脱毛器 家庭用", "サファイア 脱毛器"] 
  },
  { 
    group: 3, 
    keywords: ["酸熱トリートメント 自宅", "ケラチン トリートメント サロン", "髪質改善 トリートメント ヘアパック", "ヘアマスク うねり"] 
  },
  { 
    group: 4, 
    keywords: ["プラム リップ ブルベ", "ブルベ冬 アイシャドウ", "パープル リップ ブルベ冬", "プラム 口紅"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 74 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 74 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch74_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 74!");
}
