...
const queries = {
  t1: 'インソール 衝撃吸収 アーチサポート 扁平足 立ち仕事',
  t2: '小顔ベルト リフトアップ フェイス 二重あご',
  t3: '電動 角質リムーバー かかと削り 足裏 ローラー',
  t4: '制汗剤 ロールオン 脇汗 ワキガ デトランス'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch145_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 145 products saved successfully to scratch/batch145_fetched.json!');
}
