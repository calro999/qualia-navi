...
const groups = [
  { 
    group: 1, 
    keywords: ["ヒト型セラミド クリーム", "セラミド 保湿クリーム 乾燥肌", "CICA セラミド クリーム"] 
  },
  { 
    group: 2, 
    keywords: ["RF 美顔器 ラジオ波", "美顔器 たるみ RF ポレーション", "高周波 美顔器 引き締め"] 
  },
  { 
    group: 3, 
    keywords: ["頭皮 保湿 ローション フケ かゆみ", "頭皮 美容液 薬用 乾燥", "スカルプ ローション 保湿"] 
  },
  { 
    group: 4, 
    keywords: ["赤リップ ブルベ冬", "ボルドー リップ ブルベ", "ブルベ冬 アイシャドウ シルバー"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 71 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 71 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch71_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 71!");
}
