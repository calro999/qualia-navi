...
async function main() {
  const data = JSON.parse(fs.readFileSync('scratch_longtail_batch4_rakuten_items.json', 'utf-8'));
  const kws = ['クリニーク ラッシュパワー マスカラ', 'オペラ マイラッシュ アドバンスト', 'デジャヴュ ラッシュアップ マスカラ', 'メイベリン スカイハイ お湯落ち'];
  let items = [];
  const seenCodes = new Set();
  for (const kw of kws) {
    const fetched = await fetchRakutenItems(kw, 4);
  ...
  data.lash_perm_mascara = items.slice(0, 10);
  fs.writeFileSync('scratch_longtail_batch4_rakuten_items.json', JSON.stringify(data, null, 2), 'utf-8');
  console.log('✅ lash_perm_mascara を王道人気マスカラ10件に更新完了！');
}
