...
async function main() {
  const data = JSON.parse(fs.readFileSync('scratch_longtail_batch5_rakuten_items.json', 'utf-8'));
  const kws = ['ヘアアイロン 耐熱 ホルダー ケース', 'シリコン 耐熱マット ヘアアイロン', 'コテ 収納 ポーチ 耐熱 吊り下げ'];
  let items = [];
  const seenCodes = new Set();
  for (const kw of kws) {
    const fetched = await fetchRakutenItems(kw, 6);
    for (const it of fetched) {
      if (!seenCodes.has(it.itemCode) && it.imageUrl && it.itemName && !it.itemName.includes('ストレート ヘアアイロン 15mm')) {
  ...
  data.heatproof_iron_pouch = items.slice(0, 10);
  fs.writeFileSync('scratch_longtail_batch5_rakuten_items.json', JSON.stringify(data, null, 2), 'utf-8');
  console.log('✅ heatproof_iron_pouch を耐熱ポーチ・マット専用アイテム10件に更新完了！');
}
