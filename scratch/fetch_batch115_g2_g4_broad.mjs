...
async function fetchG(id, keyword, outFile) {
  ...
  const url = `https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260401?applicationId=${APP_ID}&accessKey=${ACCESS_KEY}&affiliateId=${AFFILIATE_ID}&keyword=${encoded}&hits=20&imageFlag=1&sort=%2BreviewCount`;
...
async function main() {
  await fetchG("g2", "キャビテーション 美容器 ボディ", "scratch/rakuten_batch115_g2_verified.json");
  await sleep(1500);
  await fetchG("g4", "角質美容液 低刺激 ピーリング 美容液", "scratch/rakuten_batch115_g4_verified.json");
}
