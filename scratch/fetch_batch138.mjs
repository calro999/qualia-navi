...
const queries = {
  t1: 'マウスウォッシュ 携帯 スティック 個包装 オクチレモン',
  t2: 'アイスローラー フェイス 冷却 美顔ローラー むくみ',
  t3: '襟 汚れ防止 テープ 汗ジミ 帽子 ファンデーション',
  t4: 'メイクスポンジ スタンド ホルダー 乾燥 パフ'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch138_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 138 products saved successfully to scratch/batch138_fetched.json!');
}
