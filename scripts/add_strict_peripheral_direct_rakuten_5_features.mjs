...
// 楽天API直接取得関数（フォールバック一切禁止・APIから取得した実データのみ使用）
...
// カニバリズムを徹底回避した、高検索ボリューム＆高購買意図の新規周辺クエリ2特集
// ① クエリ：肘 膝 黒ずみ クリーム, 肘の黒ずみ ピーリング, 膝の黒ずみ 重曹（ひじ・ひざ・かかとの黒ずみ角質ケア）
// ② クエリ：背中ニキビ スプレー, 背中ニキビ ボディソープ 薬用, 背中ニキビ跡 美白（背中・胸元のボディニキビ・ざらつきケア）
const NEW_STRICT_PERIPHERAL_FEATURES = [
  {
    ...
    introText: '「半袖やショート丈を着たとき、肘や膝の黒ずみ・ゴワつきが恥ずかしい」「ひざの黒ずみをセルフケアで早く明るくしたい」…厚くなった古い角質を柔らかく溶かすフルーツ酸（AHA）、尿素、美白有効成分プラセンタ、トラネキサム酸配合でしっとり発光する滑らかな肌へ整える楽天売れ筋肘・膝ケア5選を徹底検証！',
    items: [
      { id: 'art-elbow-himecoto-shiro-yubi-hime', keyword: '白ひじ姫 肘 黒ずみ', label: '重曹とフルーツ酸配合で肘の黒ずみ古い角質をポロポロ落とすパーツ専用ピーリング' },
      { id: 'art-elbow-rohto-mentholatum-zarapro', keyword: 'ザラプロ メンソレータム 尿素', label: '高濃度尿素が肘や膝の硬くなった角質をやわらげてツルスベ肌へ導く第3類医薬品' },
      { id: 'art-elbow-pinole-knee-bright-pack', keyword: '膝 黒ずみ ピーリング パック', label: '塗って剥がすだけで古い角質と汚れを吸着オフする集中美白パック' },
      { id: 'art-elbow-ilb-whitening-body-gel', keyword: '薬用 美白 ボディジェル プラセンタ', label: 'メラニン生成を抑えて肘・膝・ビキニラインの黒ずみを予防する薬用美白ジェル' },
      { id: 'art-elbow-house-of-rose-body-smoother', keyword: 'ハウスオブローゼ オーベイビー ボディスムーザー', label: '温泉水配合のスクラブ粒子が体温でとろけて肘膝のザラつきをつるつるにする大定番' }
    ]
  },
  {
    featureId: 'feature-5-back-acne-spray-body-soap',
    title: '【2026年最新・楽天最安値】【背中ニキビ＆デコルテざらつき撃退】手の届かない背中のブツブツ・赤み・毛穴詰まりをリセットする「神背中ニキビケア」5選',
    category: 'bodycare',
    categoryLabel: '🌿 【神背中ニキビケア5選】逆さ噴射ミスト＆薬用ソープ特集',
    introText: '「背中が開いた服や水着を着たいのに背中ニキビやニキビ跡が気になる」「手が届かない背中のケアを簡単に毎日続けたい」…逆さ噴射可能な薬用ミスト、サリチル酸、イソプロピルメチルフェノール、CICA鎮静成分配合で背中のマラセチア菌・アクネ菌を根本殺菌する楽天大人気背中ニキビケア5選を徹底解説！',
    items: [
      { id: 'art-back-orbis-clear-body-smooth-lotion', keyword: 'オルビス クリアボディ スムースローション', label: '逆さでも使える特殊スプレー！メントールの清涼感と生薬エキスで背中ニキビを防ぐ絶対定番' },
      { id: 'art-back-for-back-medicated-soap', keyword: 'ペリカン石鹸 薬用 フォーバック', label: '炭・泥・パパイン酵素で背中の毛穴詰まりとニオイを洗い流す大ヒット薬用石鹸' },
      { id: 'art-back-sana-esteny-acne-body-mist', keyword: 'サナ エステニー 薬用ボディミスト', label: 'サリチル酸とアラントイン配合で背中・デコルテのざらつきと赤みを集中ケア' },
      { id: 'art-back-rohto-acnes-25-spray', keyword: 'メンソレータム アクネス25 メディカルミスト', label: '角質軟化作用と抗炎症作用で大人のしつこい背中ニキビを治療する医薬品ミスト' },
      { id: 'art-back-somebymi-aha-bha-pha-body-cleanser', keyword: 'サムバイミー ボディクレンザー ニキビ', label: 'AHA・BHA・PHA配合で古い角質と過剰皮脂を優しくピーリングする韓国神ボディソープ' }
    ]
  }
];

async function main() {
  console.log('🚀 楽天公式OpenAPI直接通信（フォールバック一切禁止・API厳格モード）による新規周辺クエリ特集記事作成を開始します...');

  const articlesJsonPath = path.resolve(process.cwd(), 'src/data/articles.json');
  let articles = JSON.parse(fs.readFileSync(articlesJsonPath, 'utf-8'));

  for (const feat of NEW_STRICT_PERIPHERAL_FEATURES) {
    console.log(`\n==================================================`);
    ...
    for (let i = 0; i < feat.items.length; i++) {
      ...
      if (!rakutenItem) {
        console.warn(`⚠️ 楽天APIから取得できなかったため、このアイテムはスキップします（フォールバック禁止）: ${itemInfo.keyword}`);
        continue;
      }
      ...
      const singleArticle = {
        ...
        pros: [
          `使用後すぐに黒ずみ・ザラつきの軽減や背中ニキビの清潔感を実感できる高機能設計`,
          `毎日のバスタイムやボディケアルーティンに無理なく組み込める快適な使用感・高い満足度`,
...
- **安心の正規品保証**: 信頼できる認証ショップからの購入で偽物リスクを完全回避。`,
  ...
  console.log(`\n🎉 楽天API直接取得（フォールバック一切禁止）による新規周辺クエリ特集記事および個別商品記事の完全追加が完了しました！`);
}
