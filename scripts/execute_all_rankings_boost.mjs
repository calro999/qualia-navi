...
console.log(`初期記事数: ${articles.length}`);
console.log(`対象クエリ数: ${queries.length}`);

// マツパ後ケア＆コーティング美容液の神記事を新規追加
...
// 記事リストの更新（lash記事の追加）
const existingIdx = articles.findIndex(a => a.id === lashArticleId);
if (existingIdx >= 0) {
  articles[existingIdx] = newLashArticle;
} else {
  articles.unshift(newLashArticle);
}

// 既存の関連重要記事に、未網羅クエリや同義語タグ・解説セクションを徹底注入
let enrichedCount = 0;
...
console.log(`✅ 記事の最適化完了！更新記事数: ${enrichedCount}, 総記事数: ${articles.length}`);
