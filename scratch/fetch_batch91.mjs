...
const queries = [
  {
    group: "g1",
    keyword: "トラネキサム酸 乳液 薬用 美白",
    minPrice: 1500,
  },
  {
    group: "g2",
    keyword: "RF 美顔器 温冷 冷却 ラジオ波",
    minPrice: 6000,
  },
  {
    group: "g3",
    keyword: "ケラチン ヘアセラム トリートメント 洗い流さない",
    minPrice: 1500,
  },
  {
    group: "g4",
    keyword: "ブルベ夏 アイシャドウ パレット ピンク",
    minPrice: 1200,
  }
];
...
async function fetchProducts(q) {
  ...
  url.searchParams.append("hits", "15");
...
async function main() {
  for (const q of queries) {
    ...
    fs.writeFileSync(`scratch/rakuten_batch91_${q.group}_verified.json`, JSON.stringify(items, null, 2), "utf8");
    console.log(`Saved ${items.length} items for ${q.group}`);
    await sleep(1500);
  }
  console.log("All queries fetched for Batch 91!");
}
