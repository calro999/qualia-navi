...
// 検索ボリューム大×購買意欲高の4大ロングテールキーワード群
const groups = [
  { 
    group: 1, 
    keywords: ["ガラクトミセス 美容液", "ナイアシンアミド 美容液", "発酵 美容液 毛穴", "ガラクトミセス トナー"] 
  },
  { 
    group: 2, 
    keywords: ["電気バリブラシ EMS", "EMS 頭皮ブラシ 美顔器", "スカルプ ブラシ 電気", "頭皮 フェイス リフトブラシ"] 
  },
  { 
    group: 3, 
    keywords: ["ヘアミルク うねり", "ペリセア トリートメント", "ヘアミルク くせ毛 保湿", "エルカラクトン ヘアミルク"] 
  },
  { 
    group: 4, 
    keywords: ["モーヴピンク リップ", "くすみローズ アイシャドウ", "ブルベ夏 リップ", "モーヴ チーク ブルベ"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Group ${g.group} total verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch69_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 69!");
}
