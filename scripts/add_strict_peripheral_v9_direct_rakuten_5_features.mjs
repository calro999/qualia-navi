...
// ① クエリ：まつ毛コーム 金属 折りたたみ マスカラ ダマ取り, まつげセパレート コーム（マスカラ後のダマ・束感を一本一本完璧なセパレート美まつ毛に整える金属製マスカラコーム）
// ② クエリ：頭皮 スカルプローション 女性用 育毛 頭皮保湿, 頭皮の乾燥 かゆみ ローション（季節の変わり目や加齢による頭皮の乾燥・フケ・かゆみを鎮静するスカルプモイスチャーローション）
const NEW_PERIPHERAL_V9_FEATURES = [
  {
    featureId: 'feature-5-metal-eyelash-comb-separator',
    title: '【2026年最新・楽天最安値】【金属製まつ毛コーム＆ダマ取りセパレート】マスカラのダマを瞬時に解消してサロン級の美扇まつ毛を作る「神まつ毛コーム」5選',
    category: 'device',
    categoryLabel: '👁️ 【神まつ毛コーム5選】ダマ取り＆極細セパレート美まつ毛特集',
    introText: '「マスカラを塗るとダマになってヒジキまつ毛になってしまう」「毛先までスーッと伸びた放射状の美しいセパレートまつ毛を作りたい」…極細ピッチのステンレス金属歯、目元のカーブに沿うラウンド形状、持ち歩きに便利な折りたたみ式設計でプロの仕上がりを再現する楽天売れ筋まつ毛コーム5選を徹底検証！',
    items: [
      { id: 'art-comb-chasty-folding-mascara-comb', keyword: 'チャスティ マスカラコーム 金属', label: '先細ステンレス歯でどんな頑固なマスカラのダマもスルリと梳かす大定番折りたたみコーム' },
      { id: 'art-comb-kai-eyelash-comb-metal', keyword: '貝印 まつ毛コーム 金属', label: '刃物メーカーならではの精密ピッチ！自まつ毛のような繊細セパレートを作る名品' },
      { id: 'art-comb-dup-eyelash-comb-separator', keyword: 'まつげ コーム 金属 折りたたみ', label: 'ポーチの中で汚れないスマート折りたたみ！マスカラ直後の必須レスキューギア' },
      { id: 'art-comb-anua-eyelash-brush-metal', keyword: 'マスカラ ダマ取り コーム', label: '目頭から目尻まで一度に梳かせる絶妙カーブ設計のステンレス製セパレーター' },
      { id: 'art-comb-metal-lash-curler-comb', keyword: 'アイラッシュコーム ステンレス', label: '静電気が起きにくく耐久性抜群！水洗いできて清潔に使い続けられるプロ愛用コーム' }
    ]
  },
  {
    featureId: 'feature-5-scalp-moisture-lotion-dry-itch',
    title: '【2026年最新・楽天最安値】【頭皮保湿ローション＆フケ・かゆみ鎮静】乾燥によるパラパラフケ・つっぱり感をしっとり潤す「神スカルプモイスチャー」5選',
    category: 'haircare',
    categoryLabel: '🫧 【神頭皮保湿ローション5選】乾燥フケ・かゆみ鎮静＆地肌うるおい特集',
    introText: '「頭皮がカサついて服の肩に白いフケが落ちる」「お風呂上がりやエアコンで頭皮がつっぱってかゆい」…セラミド、ヒアルロン酸、ユーカリエキス、薬用消炎成分グリチルリチン酸2K配合で、ベタつかず頭皮のバリア機能を整える楽天大人気頭皮用保湿ローション5選を徹底解説！',
    items: [
      { id: 'art-scalplotion-curel-scalp-moisture-lotion', keyword: 'キュレル 頭皮保湿ローション', label: 'セラミド機能成分配合のダイレクトタッチノズルで敏感な頭皮をしっとり落ち着かせる国民的定番' },
      { id: 'art-scalplotion-yanagiya-ceramide-scalp', keyword: '柳屋 セラプリエ 薬用 頭皮保湿 育毛ローション', label: '敏感肌用薬用処方！3種のセラミドが頭皮の乾燥フケとかゆみを防ぎ育毛環境を整える' },
      { id: 'art-scalplotion-mediquick-h-scalp-lotion', keyword: 'メディクイックH 頭皮しっとりローション', label: '抗炎症・殺菌成分配合で頭皮の赤み・しつこいかゆみをスーッと鎮静する医薬品メーカー品' },
      { id: 'art-scalplotion-minon-scalp-lotion', keyword: '頭皮 保湿 ローション セラミド', label: '9種のアミノ酸と高保水成分が洗髪後のデリケートな地肌に潤いを補給する' },
      { id: 'art-scalplotion-uka-scalp-re-serum', keyword: '頭皮用 美容液 かゆみ フケ', label: 'ボタニカルエキスとヒアルロン酸配合で頭皮の油水分バランスを整える高機能エッセンス' }
    ]
  }
];

async function main() {
  console.log('🚀 楽天公式OpenAPI直接通信（フォールバック一切禁止・周辺クエリ第9弾）を開始します...');

  const articlesJsonPath = path.resolve(process.cwd(), 'src/data/articles.json');
  let articles = JSON.parse(fs.readFileSync(articlesJsonPath, 'utf-8'));

  for (const feat of NEW_PERIPHERAL_V9_FEATURES) {
    console.log(`\n==================================================`);
    ...
    for (let i = 0; i < feat.items.length; i++) {
      ...
      const singleArticle = {
        ...
        pros: [
          `使用後すぐにダマのないセパレートまつ毛や頭皮のつっぱり感・かゆみ解消を実感できる高機能設計`,
          `毎日のアイメイクや入浴後のスカルプケアルーティンに無理なく組み込める快適な使用感・高い満足度`,
...
- **安心の正規品保証**: 信頼できる認証ショップからの購入で偽物リスクを完全回避。`,
        ...
        reviewerRole: 'アイメイク＆スカルプケアアナリスト',
        summaryKeyPoints: [
    ...
    const featureArticle = {
      ...
      reviewerRole: 'シニアアイメイク＆スカルプアナリスト',
      summaryKeyPoints: [
  ...
  console.log(`\n🎉 楽天API直接取得（フォールバック一切禁止）による周辺クエリ第9弾特集記事および個別商品記事の完全追加が完了しました！`);
}
