...
const NEW_FEATURES_DATA = [
  // 特集①：【小顔＆リフトアップ・むくみ撃退】朝のフェイスライン復活！「たるみ・二重あご解消ギア＆コスメ」5選
  {
    featureId: 'feature-5-face-lift-anti-edema',
    title: '【2026年最新・楽天最安値】【小顔＆リフトアップ・むくみ撃退】朝のフェイスライン復活！「たるみ・二重あご解消ギア＆コスメ」5選',
    category: 'device',
    categoryLabel: '⚡ 【小顔リフト5選】むくみ・たるみ解消特集',
    introText: '「朝起きると顔がパンパンにむくんでいる」「フェイスラインのもたつき・二重あごが気になる」…朝のわずか3分でスッキリ引き締まったフェイスラインとシャープな小顔を取り戻す、楽天市場で大人気の最新小顔ギア＆リフトアップコスメ5選を徹底検証！',
    items: [
      { id: 'art-lift-panasonic-vitalift', keyword: 'バイタリフト かっさ', label: 'EMS×温感かっさで深層筋肉へアプローチ' },
      { id: 'art-lift-refa-carat-ray', keyword: 'リファ カラットレイ', label: 'マイクロカレント×深層ドレナージュローラー' },
      { id: 'art-lift-clarins-v-serum', keyword: 'クラランス Vコントアセラム', label: '肌引き締め植物エキス配合Vフェイス美容液' },
      { id: 'art-lift-elixir-aesthetic-serum', keyword: 'エリクシール エステティックセラム', label: 'マッサージローラー付きハリ密着美容液' },
      { id: 'art-lift-ipsa-serum-0', keyword: 'イプサ セラム 0', label: '老廃物や水分停滞を流すゼロ素肌美容液' }
    ]
  },
  // 特集②：【透明感＆白玉美肌】くすみ・黄ぐすみ・メラニン色素沈着を消し去る「高純度ブライトニング」5選
  {
    featureId: 'feature-5-brightening-glass-skin',
    title: '【2026年最新・楽天最安値】【透明感＆白玉美肌】くすみ・黄ぐすみ・メラニン色素沈着を消し去る「高純度ブライトニング」5選',
    category: 'skincare',
    categoryLabel: '✨ 【白玉透明感5選】高純度ブライトニング特集',
    introText: '「肌がどんより暗くくすんで疲れて見える」「夕方の黄ぐすみをリセットして発光するような透明感が欲しい」…高濃度ビタミンC、トラネキサム酸、グルタチオン発想の最新処方で、内側から輝く白玉美肌へ導く楽天売れ筋ブライトニングコスメ5選を徹底解説！',
    items: [
      { id: 'art-bright-haku-melano-focus', keyword: 'HAKU メラノフォーカス', label: '2種の美白有効成分がシミの根本原因へ直撃' },
      { id: 'art-bright-decorte-whitelogist', keyword: 'コスメデコルテ ホワイトロジスト', label: 'コウジ酸配合で黄ぐすみ・メラニンを一掃' },
      { id: 'art-bright-kiehls-clearly-corrective', keyword: 'キールズ DS クリアリーホワイト', label: '活性型ビタミンC×サリチル酸の透明感美容液' },
      { id: 'art-bright-transino-essence', keyword: 'トランシーノ 薬用メラノシグナルエッセンス', label: 'トラネキサム酸がメラニン生成初期をブロック' },
      { id: 'art-bright-obagi-c25-neo', keyword: 'オバジC25セラム ネオ', label: '極限濃度ピュアビタミンC25%で速攻キメ毛穴透明感' }
    ]
  }
];

async function main() {
  console.log('🚀 楽天公式OpenAPI直接通信による「新5選まとめ特集記事」作成を開始します...');

  const articlesJsonPath = path.resolve(process.cwd(), 'src/data/articles.json');
  let articles = JSON.parse(fs.readFileSync(articlesJsonPath, 'utf-8'));

  for (const feat of NEW_FEATURES_DATA) {
    console.log(`\n==================================================`);
    ...
    for (let i = 0; i < feat.items.length; i++) {
      ...
      const singleArticle = {
        ...
        pros: [
          `塗った直後から肌やフェイスラインに確かな変化を実感できる高機能設計`,
          `べたつかず肌にすっとなじみ、毎日のケアに無理なく組み込める快適な使い心地`,
          `楽天市場のポイント高還元を活用することで実質最安値での継続が可能`
        ],
        cons: [
          `大人気アイテムのためセール期やキャンペーン時には一時的に欠品する場合がある`,
          `正しく継続して効果を最大化するために推奨手順を守ることが推奨される`
        ],
...
本製品が持つ独自の処方・機能（${itemInfo.label}）が、多くのユーザーから「手放せない神コスメ」と評価される理由を徹底解説します。

---

## 2. 【製品特徴とメカニズム】他製品との圧倒的違い
1. **高浸透＆高密着アプローチ**: お悩みの深部までスムーズに成分を届ける独自デリバリー。
2. **肌への優しさと持続性**: デリケートな肌でも毎日心地よく使える低刺激設計。
...
- **安心の正規品保証**: 信頼できる認証ショップからの購入で偽物リスクを完全回避。`,
        ...
        reviewerRole: 'コスメコンシェルジュ＆コスメアナリスト',
        summaryKeyPoints: [
        ...
        faqs: [
          {
            question: `${itemInfo.keyword}は敏感肌でも毎日使えますか？`,
            answer: `はい、肌への負担に配慮した設計となっておりますので、デイリーケアとして安心してお使いいただけます。`
          }
        ]
      };
    ...
    fetchedItems.forEach((it, idx) => {
      ...
      reviewBodyContent += `楽天市場で圧倒的な支持を集める理由はその確かな実感力。「${it.label}」により、使った翌朝から違いを実感できると大絶賛されています。\n`;
  ...
  console.log(`\n🎉 楽天API直接取得による「新5選まとめ特集記事」および個別商品記事の完全追加が完了しました！`);
}
