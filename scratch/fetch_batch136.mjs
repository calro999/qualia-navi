...
const queries = {
  t1: '塩化マグネシウム 入浴剤 バスフレーク',
  t2: 'ブラシクリーナー ドライ スポンジ メイク',
  t3: 'ハイライト パウダー ツヤ パール 立体感',
  t4: '安眠 イヤーマフ 防音 遮音 睡眠'
};
...
async function main() {
  ...
  for (const [key, q] of Object.entries(queries)) {
    ...
    await sleep(1500); // 1.5秒スリープを入れてレートリミットを回避
  }

  fs.writeFileSync('scratch/batch136_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 136 products saved successfully to scratch/batch136_fetched.json!');
}
