...
const groups = [
  { 
    group: 1, 
    keywords: ["グリシルグリシン 美容液", "グリシルグリシン 化粧水", "毛穴 引き締め 美容液", "すり鉢毛穴 美容液"] 
  },
  { 
    group: 2, 
    keywords: ["目元 美顔器 EMS", "アイマッサージャー 温感", "ホットアイマスク 充電式", "目元 ケア 美顔器"] 
  },
  { 
    group: 3, 
    keywords: ["炭 シャンプー 頭皮", "泥シャンプー クレイ", "スカルプシャンプー 炭 泥", "頭皮 臭い 炭 シャンプー"] 
  },
  { 
    group: 4, 
    keywords: ["ピーチ リップ イエベ春", "コーラルピンク リップ イエベ", "イエベ春 リップ ピーチ", "多幸感 リップ"] 
  }
];
...
async function run() {
  for (const g of groups) {
    ...
    for (const kw of g.keywords) {
      ...
      await sleep(1500);
      console.log(`Batch 77 Group ${g.group}: Searching "${kw}"...`);
    ...
    console.log(`>>> Batch 77 Group ${g.group} verified: ${valid.length}`);
    fs.writeFileSync(`scratch/rakuten_batch77_g${g.group}_verified.json`, JSON.stringify(valid, null, 2));
  }
  console.log("Finished fetching all 4 groups for Batch 77!");
}
