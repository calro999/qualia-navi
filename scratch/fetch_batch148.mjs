...
const queries = {
  t1: 'ヒートレス カーラー シルク カール ヘアバンド',
  t2: 'マウスピース 歯ぎしり 食いしばり 睡眠',
  t3: '靴 消臭 粉 パウダー グランズレメディ',
  t4: '靴擦れ 防止 かかと クッション パッド テープ'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch148_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 148 products saved successfully to scratch/batch148_fetched.json!');
}
