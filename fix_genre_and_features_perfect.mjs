...
console.log('🚀 [Genre-Locked Dedicated Beauty Regenerator] コスメ・美容ジャンル限定＆重複ゼロ＆魅力特徴生成開始...');

const APP_ID = '1a3cdfd9-2aec-4b42-8290-1c53603b0012';
const ACCESS_KEY = 'pk_XkZ5h9MDKSsuVr6T5CnLnlVNvFg3hiR5vMDGrQ75cU5';
const AFFILIATE_ID = '54d2a438.4bc4abc2.54d2a439.aa1be583';

// 楽天市場「美容・コスメ・香水」ジャンルID: 100938
const BEAUTY_GENRE_ID = '100938';
...
// 楽天APIから「美容コスメジャンル(100938)」限定で重複なく10商品を取得
async function fetchBeautyItemsDirect(keywords, minHits = 10) {
  const distinct = [];
  ...
  for (const kw of keywords) {
    ...
    try {
      // genreId=100938 (美容・コスメ・香水) を指定して非コスメの混入を100%防止
      const url = `https://openapi.rakuten.co.jp/ichibams/api/IchibaItem/Search/20260401?applicationId=${APP_ID}&accessKey=${ACCESS_KEY}&affiliateId=${AFFILIATE_ID}&genreId=${BEAUTY_GENRE_ID}&keyword=${encodeURIComponent(kw)}&format=json&hits=30&sort=-reviewCount&imageFlag=1`;
      ...
      console.warn(`⚠️ 通信例外 (${kw}): ${err.message}`);
    }
...
function generateProductFeaturesText(item, articleTitle, rank) {
  ...
  } else if (articleTitle.includes('カラーシャンプー') || articleTitle.includes('紫シャンプー') || articleTitle.includes('ピンクシャンプー') || articleTitle.includes('染め')) {
  ...
  } else if (articleTitle.includes('頭皮') || articleTitle.includes('スカルプ') || articleTitle.includes('シャンプー') || articleTitle.includes('ヘア')) {
...
async function main() {
  ...
  console.log(`🔍 対象の10選コンテンツ記事: 全${content10s.length}件 (コスメジャンル限定検索)`);

  let processed = 0;
  for (const art of content10s) {
    const idx = articlesData.findIndex(a => a.id === art.id);
    console.log(`\n----------------------------------------`);
    console.log(`🔨 [${processed + 1}/${content10s.length}] コスメジャンル確定取得: [${art.id}] ${art.title.slice(0, 35)}...`);

    // クエリ最適化
    let cleanKw = art.title
      .replace(/【.*?】/g, '')
      .replace(/\[.*?\]/g, '')
      .replace(/[！!？?・&％%]/g, ' ')
      .replace(/\d+選.*/, '')
    ...
    const tagsKws = (art.tags || []).slice(0, 3);
    const keywords = [
      tagsKws.join(' '),
      cleanKw,
      cleanKw.split(' ').slice(0, 2).join(' '),
      cleanKw.split(' ')[0],
      ...(art.tags || [])
    ].filter(k => k && k.length >= 2);

    const items = await fetchBeautyItemsDirect(keywords, 10);
    if (items.length < 10) {
      console.warn(`⚠️ 10件取得不足 (${items.length}件): ${art.id}`);
      continue;
    }

    articlesData[idx] = buildRefreshedContent(art, items);
    console.log(`✅ コスメ限定・重複完全排除・魅力特徴付きで10商品更新完了！ (1位: ${items[0].itemName.slice(0, 25)}...)`);
    processed++;
  ...
  console.log(`\n🎉 全${processed}件の10選記事においてコスメジャンル限定・重複完全排除・本物の特徴付与が完了しました！`);
}
