...
async function run() {
  const g4 = await fetchRakutenItems('アイシャドウ パレット カーキ アースカラー', 10);
  if (g4.length < 10) {
    const extra = await fetchRakutenItems('アイシャドウ パレット オリーブ', 10);
    for (const it of extra) {
      if (g4.length < 10 && !g4.find(x => x.name === it.name)) {
        g4.push(it);
      }
    }
  }
