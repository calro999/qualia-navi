...
const queries = {
  t1: '足裏 角質 パック ピーリング',
  t2: 'アイシャドウブラシ セット 馬毛 携帯用',
  t3: 'フレンチ ネイル スタンパー シリコン',
  t4: '首 シワ クリーム 首元 ネッククリーム'
};
...
async function main() {
  ...
  fs.writeFileSync('scratch/batch167_fetched.json', JSON.stringify(allResults, null, 2), 'utf-8');
  console.log('Batch 167 products saved successfully to scratch/batch167_fetched.json!');
}
